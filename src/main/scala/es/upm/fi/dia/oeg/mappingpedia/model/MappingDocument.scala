...
import es.upm.fi.dia.oeg.mappingpedia.utility.{MappingPediaUtility, MpcUtility}
...
class MappingDocument(val dctIdentifier:String) {
  ...
  */
  ...
  def setFile(pFile1:MultipartFile, pFile2:MultipartFile, datasetId:String) = {
    if(pFile1 != null) {
      this.mappingDocumentFile = MpcUtility.multipartFileToFile(pFile1, datasetId);}
    else if(pFile2 != null) {
      this.mappingDocumentFile = MpcUtility.multipartFileToFile(pFile2 , datasetId);
    }
  }
